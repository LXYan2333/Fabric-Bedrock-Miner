...
object ServerCommands {
    fun register() {
        CommandCompat.register { dispatcher ->
            dispatcher.register(
                Commands.literal("bedrock-miner")
                    .requires { source -> CommandCompat.requiresAdmin(source) }.then(
                                ...
                                .then(CommandCompat.idArgument("block").suggests { _, builder ->
                                    ...
                                    if (!ServerConfigManager.addToBlockList(block)) {
                                        context.source.sendFailure(
                                            CommandCompat.literal("Invalid block name: $block")
                                        )
                                        return@executes 0
                                    }
                                    CommandCompat.sendSuccess(context.source, CommandCompat.literal("Added $block to block list"), true)
                                    1
                                })
                        ).then(
                            Commands.literal("remove")
                                .then(CommandCompat.idArgument("block").suggests { _, builder ->
                                    ...
                                    if (!ServerConfigManager.removeFromBlockList(block)) {
                                        context.source.sendFailure(
                                            CommandCompat.literal("Block $block is not in block list")
                                        )
                                        return@executes 0
                                    }
                                    CommandCompat.sendSuccess(context.source, CommandCompat.literal("Removed $block from block list"), true)
                                    1
                                })
                        ).then(
                            Commands.literal("clear").executes { context ->
                                ServerConfigManager.clearBlockList()
                                CommandCompat.sendSuccess(context.source, CommandCompat.literal("Cleared block list"), true)
                                1
                            }).then(
                            Commands.literal("list").executes { context ->
                                val list = ServerConfigData.serverBlockList.ifEmpty { "(empty)" }
                                CommandCompat.sendSuccess(context.source, CommandCompat.literal("Block list: $list"), false)
                                1
                            })
                    ).then(
                        Commands.literal("allowlist").then(
                            Commands.literal("add")
                                .then(CommandCompat.idArgument("block").suggests { _, builder ->
                                    ...
                                    CommandCompat.sendSuccess(context.source, CommandCompat.literal("Added $block to allow list"), true)
                                    1
                                })
                        ).then(
                            Commands.literal("remove")
                                .then(CommandCompat.idArgument("block").suggests { _, builder ->
                                    ...
                                    if (!ServerConfigManager.removeFromAllowList(block)) {
                                        context.source.sendFailure(
                                            CommandCompat.literal("Block $block is not in allow list")
                                        )
                                        return@executes 0
                                    }
                                    CommandCompat.sendSuccess(context.source, CommandCompat.literal("Removed $block from allow list"), true)
                                    1
                                })
                        ).then(
                            Commands.literal("clear").executes { context ->
                                ServerConfigManager.clearAllowList()
                                CommandCompat.sendSuccess(context.source, CommandCompat.literal("Cleared allow list"), true)
                                1
                            }).then(
                            Commands.literal("list").executes { context ->
                                val list = ServerConfigData.serverAllowList.ifEmpty { "(empty)" }
                                CommandCompat.sendSuccess(context.source, CommandCompat.literal("Allow list: $list"), false)
                                1
                            })
                    ).then(
                        Commands.literal("mode")
                            .then(Commands.argument("mode", StringArgumentType.word()).suggests { _, builder ->
                                ...
                                if (mode != "BLOCKED" && mode != "ALLOWED") {
                                    context.source.sendFailure(
                                        CommandCompat.literal("Invalid mode. Use BLOCKED or ALLOWED.")
                                    )
                                    return@executes 0
                                }
                                ServerConfigManager.setBlockListMode(mode)
                                CommandCompat.sendSuccess(context.source, CommandCompat.literal("Set block list mode to $mode"), true)
                                1
                            })
                    ).then(
                        Commands.literal("reload").executes { context ->
                            ...
                            CommandCompat.sendSuccess(context.source, CommandCompat.literal("Reloaded server config from disk"), true)
                            1
                        }).then(
                        Commands.literal("show").executes { context ->
                            CommandCompat.sendSuccess(
                                context.source,
                                CommandCompat.literal("§7Block list:§r ${ServerConfigData.serverBlockList.ifEmpty { "(empty)" }}"),
                                false,
                            )
                            CommandCompat.sendSuccess(
                                context.source,
                                CommandCompat.literal("§7Allow list:§r ${ServerConfigData.serverAllowList.ifEmpty { "(empty)" }}"),
                                false,
                            )
                            CommandCompat.sendSuccess(
                                context.source,
                                CommandCompat.literal("§7Block list mode:§r ${ServerConfigData.serverBlockListMode}"),
                                false,
                            )
                            CommandCompat.sendSuccess(
                                context.source,
                                CommandCompat.literal("§7Protocol version:§r ${ServerConfigData.PROTOCOL_VERSION}"),
                                false,
                            )
                            1
                        })
